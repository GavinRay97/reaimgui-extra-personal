...
void ShowFileDialog(bool* open, char* buffer, unsigned int bufferSize, FileDialogType type = FileDialogType::OpenFile)
{
    ...
    if (open)
    {
        ...
        {
            if (ImGui::Selectable(folders[i].path().stem().string().c_str(), i == fileDialogFolderSelectIndex,
        ...
        {
            if (ImGui::Selectable(files[i].path().filename().string().c_str(), i == fileDialogFileSelectIndex,
                                  ImGuiSelectableFlags_AllowDoubleClick,
                                  ImVec2(ImGui::GetWindowContentRegionWidth(), 0)))
            {
        ...
        std::string selectedFilePath =
            fileDialogCurrentPath + (fileDialogCurrentPath.back() == '\\' ? "" : "\\") +
            ...
            if (ImGui::Button("Create##1"))
            {
                ...
                else
                {
                    std::string newFilePath =
                        fileDialogCurrentPath + (fileDialogCurrentPath.back() == '\\' ? "" : "\\") + newFolderName;
        ...
        if (ImGui::BeginPopup("DeleteFolderPopup", ImGuiWindowFlags_Modal))
        {
            ...
            if (ImGui::Button("Yes"))
            {
                std::filesystem::remove(fileDialogCurrentPath + (fileDialogCurrentPath.back() == '\\' ? "" : "\\") +
                                        fileDialogCurrentFolder);
                ImGui::CloseCurrentPopup();
            }
            ImGui::SameLine();
        ...
        if (ImGui::Button("Choose"))
        {
            if (type == FileDialogType::SelectFolder)
            {
                ...
                {
                    strcpy(buffer, (fileDialogCurrentPath + (fileDialogCurrentPath.back() == '\\' ? "" : "\\") +
                                    fileDialogCurrentFolder)
                                       .c_str());
                    fileDialogFileSelectIndex = 0;
                    fileDialogFolderSelectIndex = 0;
