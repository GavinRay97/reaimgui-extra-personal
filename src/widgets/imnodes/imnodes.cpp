// TAKEN FROM COMMIT: https://github.com/Nelarius/imnodes/commit/e9c4c5b942a2de55d8a574c18aa39f57479d4118
...
struct NodeData
{
    ...
    NodeData(const int node_id)
        : id(node_id), origin(100.0f, 100.0f), title_bar_content_rect(), rect(ImVec2(0.0f, 0.0f), ImVec2(0.0f, 0.0f)),
          color_style(), layout_style(), pin_indices(), draggable(true)
    {
    }
...
struct PinData
{
    ...
    PinData(const int pin_id)
        : id(pin_id), parent_node_idx(), attribute_rect(), type(AttributeType_None), shape(PinShape_CircleFilled),
          pos(), flags(AttributeFlags_None), color_style()
    {
    }
};
...
inline ImVec2 eval_bezier(float t, const BezierCurve& bezier)
{
    ...
    return ImVec2((1 - t) * (1 - t) * (1 - t) * bezier.p0.x + 3 * (1 - t) * (1 - t) * t * bezier.p1.x +
...
inline bool rectangle_overlaps_line_segment(const ImRect& rect, const ImVec2& p1, const ImVec2& p2)
{
    ...
    const int corner_signs[4] = {sign(eval_implicit_line_eq(p1, p2, flip_rect.Min)),
...
struct EditorContext
{
    ...
    EditorContext()
        : nodes(), pins(), links(), panning(0.f, 0.f), selected_node_indices(), selected_link_indices(),
          click_interaction_type(ClickInteractionType_None), click_interaction_state()
    {
    }
};
...
{
    g->node_idx_to_submission_idx.SetInt(static_cast<ImGuiID>(node_idx), g->node_idx_submission_order.Size);
...
{
    g->canvas_draw_list->_Splitter.SetCurrentChannel(g->canvas_draw_list, g->canvas_draw_list->_Splitter._Count - 1);
}
...
template <typename T> int object_pool_find(ObjectPool<T>& objects, const int id)
{
    const int index = objects.id_map.GetInt(static_cast<ImGuiID>(id), -1);
    return index;
}

template <typename T> void object_pool_update(ObjectPool<T>& objects)
{
...
template <> void object_pool_update(ObjectPool<NodeData>& nodes)
{
...
template <typename T> void object_pool_reset(ObjectPool<T>& objects)
{
...
template <typename T> int object_pool_find_or_create_index(ObjectPool<T>& objects, const int id)
{
...
template <> int object_pool_find_or_create_index(ObjectPool<NodeData>& nodes, const int node_id)
{
...
template <typename T> T& object_pool_find_or_create_object(ObjectPool<T>& objects, const int id)
{
...
void begin_link_detach(EditorContext& editor, const int link_idx, const int detach_pin_idx)
{
    ...
    state.link_creation.start_pin_idx = detach_pin_idx == link.start_pin_idx ? link.end_pin_idx : link.start_pin_idx;
...
{
    const bool any_ui_element_hovered = g->hovered_node_idx.has_value() || g->hovered_link_idx.has_value() ||
...
void box_selector_update_selection(EditorContext& editor, ImRect box_rect)
{
    ...
    for (int link_idx = 0; link_idx < editor.links.pool.size(); ++link_idx)
    {
        if (editor.links.in_use[link_idx])
        {
            ...
            const ImVec2 start =
                get_screen_space_pin_coordinates(node_start_rect, pin_start.attribute_rect, pin_start.type);
            const ImVec2 end = get_screen_space_pin_coordinates(node_end_rect, pin_end.attribute_rect, pin_end.type);
...
bool should_link_snap_to_pin(const EditorContext& editor, const PinData& start_pin, const int hovered_pin_idx,
    ...
    case ClickInteractionType_LinkCreation: {
        ...
        const OptionalIndex maybe_duplicate_link_idx =
            g->hovered_pin_idx.has_value()
                ? find_duplicate_link(editor, editor.click_interaction_state.link_creation.start_pin_idx,
                                      g->hovered_pin_idx.value())
                : OptionalIndex();

        const bool should_snap =
            g->hovered_pin_idx.has_value() &&
            should_link_snap_to_pin(editor, start_pin, g->hovered_pin_idx.value(), maybe_duplicate_link_idx);
        ...
        const ImVec2 end_pos =
            should_snap ? get_screen_space_pin_coordinates(editor, editor.pins.pool[g->hovered_pin_idx.value()])
                        : g->mouse_pos;

        const LinkBezierData link_data =
            get_link_renderable(start_pos, end_pos, start_pin.type, g->style.link_line_segments_per_length);
...
OptionalIndex resolve_hovered_link(const ObjectPool<LinkData>& links, const ObjectPool<PinData>& pins)
{
    ...
    for (int idx = 0; idx < links.pool.Size; ++idx)
    {
        ...
        const LinkBezierData link_data =
            get_link_renderable(start_pin.pos, end_pin.pos, start_pin.type, g->style.link_line_segments_per_length);
        ...
        // The distance test
        {
            ...
            if (link_rect.Contains(g->mouse_pos))
            {
                const float distance =
                    get_distance_to_cubic_bezier(g->mouse_pos, link_data.bezier, link_data.num_segments);
...
void draw_pin_shape(const ImVec2& pin_pos, const PinData& pin, const ImU32 pin_color)
{
    ...
    case PinShape_CircleFilled: {
        g->canvas_draw_list->AddCircleFilled(pin_pos, g->style.pin_circle_radius, pin_color, circle_num_segments);
    }
    break;
    case PinShape_Quad: {
...
void draw_pin(EditorContext& editor, const int pin_idx, const bool left_mouse_clicked)
{
    ...
    const bool pin_hovered =
        g->hovered_pin_idx == pin_idx && editor.click_interaction_type != ClickInteractionType_BoxSelection;
...
void draw_node(EditorContext& editor, const int node_idx)
{
    ...
    const bool node_hovered =
        g->hovered_node_idx == node_idx && editor.click_interaction_type != ClickInteractionType_BoxSelection;
...

    {
...
#else
            g->canvas_draw_list->AddRectFilled(title_bar_rect.Min, title_bar_rect.Max, titlebar_background,
                                               node.layout_style.corner_rounding, ImDrawFlags_RoundCornersTop);
        ...
        if ((g->style.flags & StyleFlags_NodeOutline) != 0)
        {
            ...
            g->canvas_draw_list->AddRect(node.rect.Min, node.rect.Max, node.color_style.outline,
                                         node.layout_style.corner_rounding, ImDrawCornerFlags_All,
...
void draw_link(EditorContext& editor, const int link_idx)
{
    ...
    const bool link_hovered =
        g->hovered_link_idx == link_idx && editor.click_interaction_type != ClickInteractionType_BoxSelection;
...
Style::Style()
    : grid_spacing(32.f), node_corner_rounding(4.f), node_padding_horizontal(8.f), node_padding_vertical(8.f),
      node_border_thickness(1.f), link_thickness(3.f), link_line_segments_per_length(0.1f), link_hover_distance(10.f),
      pin_circle_radius(4.f), pin_quad_side_length(7.f), pin_triangle_side_length(9.5), pin_line_thickness(1.f),
      pin_hover_radius(10.f), pin_offset(0.f), flags(StyleFlags(StyleFlags_NodeOutline | StyleFlags_GridLines)),
      colors()
{
}
...
void BeginNodeEditor()
{
    ...
    ImGui::BeginGroup();
    {
...

        {
            ...
            g->canvas_rect_screen_space =
                ImRect(editor_space_to_screen_space(ImVec2(0.f, 0.f)), editor_space_to_screen_space(canvas_size));
...
bool IsLinkDropped(int* const started_at_id, const bool including_detached_links)
{
    ...
    const bool link_dropped =
        (g->element_state_change & ElementStateChange_LinkDropped) != 0 &&
