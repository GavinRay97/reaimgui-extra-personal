...
PlotStatus Plot(const char* label, const PlotConfig& conf)
{
    ...
    if (conf.values.count > 0)
    {
        ...
        if (conf.grid_x.show)
        {
            ...
            case PlotConfig::Scale::Linear: {
                ...
                for (int i = 0; i <= cnt; ++i)
                {
                    ...
                    window->DrawList->AddLine(ImVec2(x0, y0), ImVec2(x0, y1),
        ...
        if (conf.grid_y.show)
        {
            ...
            for (int i = 0; i <= cnt; ++i)
            {
                ...
                window->DrawList->AddLine(ImVec2(x0, y0), ImVec2(x1, y0),
    ...
    if (conf.overlay_text)
        RenderTextClipped(ImVec2(frame_bb.Min.x, frame_bb.Min.y + style.FramePadding.y), frame_bb.Max,
                          conf.overlay_text, NULL, NULL, ImVec2(0.5f, 0.0f));
