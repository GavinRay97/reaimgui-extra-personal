...
{ H2, true }, { H3, false } }, NULL }; mdConfig.linkCallback =         LinkCallback; mdConfig.tooltipCallback = NULL;
...
// - linkIcon is a string which encode a "Link" icon, if available in the current font (e.g. linkIcon = ICON_FA_LINK
// with FontAwesome + IconFontCppHeaders https://github.com/juliettef/IconFontCppHeaders)
            ...
            if (c == ')')
            {
                ...
                if (link.isImage) // it's an image, render it.
                {
                    ...
                    if (mdConfig_.imageCallback)
                    {
                        MarkdownImageData imageData = mdConfig_.imageCallback(
                            {markdown_ + link.text.start, link.text.size(), markdown_ + link.url.start, link.url.size(),
                    ...
                    if (ImGui::IsItemHovered())
                    {
                        ...
                        {
                            mdConfig_.linkCallback({markdown_ + link.text.start, link.text.size(),
                ...
                else // it's a link, render it.
                {
                    textRegion.RenderLinkTextWrapped(markdown_ + link.text.start,
                                                     markdown_ + link.text.start + link.text.size(), link, style,
...
inline void TextRegion::RenderLinkTextWrapped(const char* text_, const char* text_end_, const Link& link_,
                                              const ImGuiStyle& style_, const char* markdown_,
